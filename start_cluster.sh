...
docker -H 10.0.7.11:2375 run -d --restart always -p 4000:4000 --name swarm_manager_01 swarm:${SWARM_VERSION} manage -H :4000 --replication --advertise 10.0.7.11:2375 consul://10.0.7.11:8500

# Run the first and second Swarm nodes
echo "Starting Swarm Agent #1..."
docker -H 10.0.7.12:2375 run -d --restart always --name swarm_agent_00 swarm:${SWARM_VERSION} join --heartbeat 20s --ttl 30s --advertise 10.0.7.12:2375 consul://10.0.7.12:8500
echo "Starting Swarm Agent #2..."
docker -H 10.0.7.13:2375 run -d --restart always --name swarm_agent_01 swarm:${SWARM_VERSION} join --heartbeat 20s --ttl 30s --advertise 10.0.7.13:2375 consul://10.0.7.13:8500
